...
class CurlBuilder
{
    ...
    private const HEADER_HTTP_CONTENT_TYPE = 'http-content-type';
    ...
    private function removeBoundaryFromContentType(array $headers): array
    {
        ...
        if (!empty($headers[self::HEADER_HTTP_CONTENT_TYPE])) {
            $headers[self::HEADER_HTTP_CONTENT_TYPE] = $this->removeBoundary($headers[self::HEADER_HTTP_CONTENT_TYPE]);
        }
